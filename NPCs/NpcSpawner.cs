...
internal unsafe sealed class NpcSpawner : IDisposable
{
    ...
    {
        if (this.npcs.TryGetValue(id, out var entry) && this.GetCharacter(entry) is not null)
        {
    ...
    public string GetAppearanceStatus(Guid id)
    {
        if (!this.npcs.TryGetValue(id, out var entry))
            return "Waiting to spawn";
        if (entry.AppearanceError != 0)
            return $"Error {entry.AppearanceError}";
        return entry.DrawState == NpcDrawState.Ready ? string.Empty : "Loading appearance...";
    }
    ...
    public IReadOnlyList<NpcNameplate> GetNameplates()
    {
        ...
        foreach (var entry in this.npcs.Values)
        {
            ...
            var character = this.GetCharacter(entry);
    ...
    {
        if (!this.npcs.TryGetValue(id, out var entry) || this.GetCharacter(entry) is null ||
    ...
    private void ApplyNpc(SpawnedFurniture item)
    {
        ...
        {
            this.appearanceInterop.ForgetActor(entry.ObjectIndex);
            this.npcs.Remove(item.Id);
            return;
        }

        this.SetNpcVisible(character, entry, true);
        if (this.UpdateDraw(character, entry))
    ...
    private bool CreateNpc(SpawnedFurniture item, out SpawnedNpc entry)
    {
        ...
        NpcActorIdentity.Initialize(character, sourceCharacter->HomeWorld);
        ...
        entry = new SpawnedNpc(clientObjectIndex, objectIndex, (nint)character, item);
    ...
    {
        var penumbraAvailable = this.appearanceInterop.PenumbraAvailable;
        if (!penumbraAvailable)
            entry.PenumbraAttempted = false;

        if (Environment.TickCount64 < entry.NextAppearanceAttemptMilliseconds)
            return;

        entry.NextAppearanceAttemptMilliseconds = Environment.TickCount64 + 1000;

        // penumbra runs first
        if (penumbraAvailable && (!entry.PenumbraAttempted || entry.LastPenumbraCollectionId != item.NpcPenumbraCollectionId))
        {
            this.ApplyPenumbraCollection(character, item, entry);
            return;
        }
        ...
        {
            entry.AppearanceError = 0;
            return;
        }

        var hasPortableState = !string.IsNullOrWhiteSpace(item.NpcGlamourerStateBase64);
        if (!hasPortableState && item.NpcGlamourerDesignId == Guid.Empty &&
            (entry.LastGlamourerStateBase64.Length > 0 || entry.LastGlamourerDesignId != Guid.Empty))
        {
            entry.RecreateRequested = true;
            return;
        }
        ...
        if (hasPortableState)
        {
            ...
            {
                if (!this.appearanceInterop.ApplyGlamourerState(
                    entry.ObjectIndex,
                    item.NpcGlamourerStateBase64))
                {
                    entry.AppearanceError = 610;
                    return;
                }
                ...
                entry.DrawState = NpcDrawState.RedrawRequested;
                entry.NextAppearanceAttemptMilliseconds = 0;
                entry.AppearanceError = 0;
                return;
            }
        }
        else
        {
            ...
            {
                var applied = item.NpcGlamourerDesignId == Guid.Empty || this.appearanceInterop.ApplyGlamourerDesign(
                    entry.ObjectIndex,
                    item.NpcGlamourerDesignId);
                if (!applied)
                {
                    entry.AppearanceError = 605;
                    return;
                }

                entry.GlamourerDesignAttempted = true;
                entry.LastGlamourerDesignId = item.NpcGlamourerDesignId;
                if (item.NpcGlamourerDesignId != Guid.Empty &&
        ...
        {
            var needsProfileUpdate = !string.IsNullOrWhiteSpace(item.NpcCustomizePlusProfileJson) ||
                !string.IsNullOrWhiteSpace(entry.LastCustomizePlusProfileJson);
            if (needsProfileUpdate && !this.appearanceInterop.ApplyCustomizePlusProfile(
                entry.ObjectIndex,
                item.NpcCustomizePlusProfileJson))
            {
                entry.AppearanceError = 611;
                return;
            }
    ...
    private void ApplyPenumbraCollection(NativeCharacter* character, SpawnedFurniture item, SpawnedNpc entry)
    {
        entry.PenumbraAttempted = false;
        ...
        character->DisableDraw();
        entry.DrawState = NpcDrawState.WaitingToEnable;
        entry.NextAppearanceAttemptMilliseconds = 0;
        entry.AppearanceError = 0;
    }
    ...
    private void OnPenumbraObjectRedrawn(nint address, int objectIndex)
    {
        foreach (var entry in this.npcs.Values)
        {
            if (entry.ObjectIndex != objectIndex || entry.Pointer != address || this.GetCharacter(entry) is null)
                continue;
    ...
    private NativeCharacter* GetCharacter(SpawnedNpc entry)
    {
        var com = ClientObjectManager.Instance();
        if (com is null)
            return null;

        var character = com->GetObjectByIndex(entry.ClientObjectIndex);
        if (character is null || (nint)character != entry.Pointer || character->ObjectIndex != entry.ObjectIndex ||
            Service.ObjectTable[entry.ObjectIndex]?.Address != entry.Pointer ||
            Service.ObjectTable.LocalPlayer?.Address == entry.Pointer)
            return null;

        var name = entry.ActorName;
        if (!character->Name[..name.Length].SequenceEqual(name) || character->Name[name.Length] != 0)
            return null;
    ...
    private bool UpdateDraw(NativeCharacter* character, SpawnedNpc entry)
    {
        if (entry.DrawState == NpcDrawState.RedrawRequested)
        {
            character->DisableDraw();
            entry.DrawState = NpcDrawState.WaitingToEnable;
            entry.LastDrawObject = 0;
            entry.CustomizePlusAttempted = false;
            entry.LastAppliedPosition = new Vector3(float.NaN, float.NaN, float.NaN);
            entry.LastAppliedYawRadians = float.NaN;
            entry.LastAppliedScale = float.NaN;
            return false;
        }

        if (entry.DrawState == NpcDrawState.WaitingToEnable)
        {
            if (character->IsReadyToDraw())
            {
                character->EnableDraw();
                entry.DrawState = NpcDrawState.WaitingForModel;
            }

            return false;
        }

        if (character->DrawObject is null || !character->DrawObject->IsVisible || !character->IsReadyToDraw())
            return false;

        if (entry.LastDrawObject != (nint)character->DrawObject)
        {
            entry.LastDrawObject = (nint)character->DrawObject;
            entry.CustomizePlusAttempted = false;
        }

        entry.DrawState = NpcDrawState.Ready;
        return true;
    }
    ...
    private sealed class SpawnedNpc(ushort clientObjectIndex, ushort objectIndex, nint pointer, SpawnedFurniture sceneItem)
    {
        ...
        public byte[] ActorName { get; } = NpcActorIdentity.Name(objectIndex);
        ...
        public NpcDrawState DrawState { get; set; }
        public nint LastDrawObject { get; set; }
        public int AppearanceError { get; set; }
        public long NextAppearanceAttemptMilliseconds { get; set; }
