...
internal sealed class PluginUi : IDisposable
{
    ...
    private const int MinimumAutosaveSeconds = 10;
    private const int MaximumAutosaveSeconds = 20 * 60;
    ...
    private long lastSceneSaveMilliseconds;
    private long autosaveNoticeUntilMilliseconds;
    ...
    public PluginUi(FurnitureCatalog catalog, MapAssetCatalog mapAssetCatalog, StainCatalog stainCatalog, InteriorFixtureCatalog interiorFixtureCatalog, SceneStore sceneStore, MyCatalogueStore myCatalogueStore, FurnitureSpawner spawner, NpcAppearanceInterop npcAppearanceInterop, NpcSpawner npcSpawner, SceneLighting lighting)
    {
        ...
        this.configuration = Service.PluginInterface.GetPluginConfig() as PluginConfiguration ?? new PluginConfiguration();
        this.configuration.AutosaveIntervalSeconds = Math.Clamp(
            this.configuration.AutosaveIntervalSeconds,
            MinimumAutosaveSeconds,
            MaximumAutosaveSeconds);
    ...
    public void Draw()
    {
        ...
        if (ImGui.BeginTabBar("main-tabs"))
        {
            ...
            if (ImGui.BeginTabItem("Placed Lights"))
            {
                this.DrawPlacedLightsTab();
                ImGui.EndTabItem();
            }
    ...
    private void DrawToolbar()
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button("Settings", new Vector2(76, 24)))
        ...
        else if (Environment.TickCount64 < this.autosaveNoticeUntilMilliseconds)
        {
            ImGui.SameLine();
            ImGui.TextDisabled("Autosaved");
        }
    }

    private void DrawSettingsPopup()
    {
        const string popupName = "Settings";
        ...
        var interval = this.configuration.AutosaveIntervalSeconds;
        ImGui.SetNextItemWidth(280);
        if (ImGui.SliderInt("Autosave interval", ref interval, MinimumAutosaveSeconds, MaximumAutosaveSeconds, "%d seconds"))
        {
            this.configuration.AutosaveIntervalSeconds = interval;
            Service.PluginInterface.SavePluginConfig(this.configuration);
        }

        ImGui.Spacing();
        if (ImGui.Button("Close", new Vector2(88, 24)))
    ...
    private void DrawClearConfirmation()
    {
        ...
        if (ImGui.Button("Clear scene", new Vector2(112, 28)))
        {
            ...
            // clearing starts a new untitled scene; the previous file is no longer an autosave target
            this.currentScenePath = string.Empty;
            this.lastSceneSaveMilliseconds = 0;
            this.autosaveNoticeUntilMilliseconds = 0;
            this.sceneStatus = "";
            ImGui.CloseCurrentPopup();
        }
    ...
    private void SaveScene(string path, bool autosave = false)
    {
        try
        {
            this.currentScenePath = this.sceneStore.Save(path, this.spawned, this.folders, this.interiorFixtures);
            this.lastSceneSaveMilliseconds = Environment.TickCount64;
            this.autosaveNoticeUntilMilliseconds = autosave
                ? this.lastSceneSaveMilliseconds + 4000
                : 0;
            this.sceneStatus = "";
        }
    ...
    private void LoadScene(string path)
    {
        try
        {
            ...
            if (this.ApplyLoadedScene(result))
            {
    ...
    private bool ApplyLoadedScene(LoadSceneResult result)
    {
        if (result.Error.Length != 0 &&
            result.Props.Count == 0 &&
            result.Folders.Count == 0 &&
            result.InteriorFixtures.Count == 0 &&
            result.Skipped == 0)
        {
    ...
    {
        if (this.currentScenePath.Length == 0)
            return;

        var now = Environment.TickCount64;
        var intervalMilliseconds = this.configuration.AutosaveIntervalSeconds * 1000L;
        if (now - this.lastSceneSaveMilliseconds < intervalMilliseconds)
            return;

        // failed saves wait for the normal interval instead of retrying every frame
        this.lastSceneSaveMilliseconds = now;
        this.SaveScene(this.currentScenePath, autosave: true);
    }
    ...
    private void DrawPlacedLightsTab()
    {
        var playerPosition = Service.ObjectTable.LocalPlayer?.Position;
        var rows = this.lighting.FindPlacedLights()
            .OrderBy(light => playerPosition is { } position
                ? Vector3.DistanceSquared(position, light.State.Position)
                : 0.0f)
            .ToArray();

        if (ImGui.BeginChild("placed-lights", new Vector2(0, 0), true))
        {
            if (rows.Length == 0)
                ImGui.TextDisabled("No placed lights loaded.");

            foreach (var light in rows)
            {
                var reference = light.Reference;
                var item = this.spawned.FirstOrDefault(entry => entry.Light?.Placed == reference);
                var removed = item is { Enabled: false };
                ImGui.PushID($"{reference.Source}-{reference.TerritoryId}-{reference.InstanceId}-{reference.SubId}");

                if (ImGui.Button(removed ? "Restore" : "Remove", new Vector2(76, 24)))
                {
                    if (removed)
                    {
                        this.RemoveSpawnedItem(item!);
                    }
                    ...
                    {
                        this.AddSpawned(new SpawnedFurniture(light) { Enabled = false });
                    }
                }
                ...
                ImGui.SameLine();
                ImGui.TextDisabled(removed ? "Removed" : light.State.Enabled ? "Visible" : "Hidden");

                var position = light.State.Position;
                var distance = playerPosition is { } player
                    ? $"  |  Distance: {Vector3.Distance(player, position):F1} m"
                    : string.Empty;
                ImGui.TextDisabled($"Position: {position.X:F1}, {position.Y:F1}, {position.Z:F1}{distance}");
                ImGui.TextDisabled($"Intensity: {light.State.Settings.Intensity:F2}  |  Range: {light.State.Settings.Range:F1} m");
                ImGui.Separator();
                ImGui.PopID();
            }
        }
    ...
    private void DrawDetails()
    {
        ...
        if (selected.IsLight)
            this.DrawLightControls(selected);
        else if (selected.IsFxEmitter)
    ...
    private void DrawLightPickerTab()
    {
        foreach (var type in Enum.GetValues<SceneLightType>())
        {
            if (ImGui.Button($"Add {SceneLighting.LightName(type)}", new Vector2(180, 28)))
            {
                this.AddSpawned(new SpawnedFurniture(type, this.DefaultSpawnPosition()));
                ImGui.CloseCurrentPopup();
            }
        }

        ImGui.Separator();
        ImGui.TextUnformatted("Placed lights");
        if (!ImGui.BeginChild("placed-light-list", new Vector2(560, 290), true))
        {
            ImGui.EndChild();
            return;
        }

        foreach (var light in this.lighting.FindPlacedLights())
        {
            var reference = light.Reference;
            ImGui.PushID($"{reference.Source}-{reference.TerritoryId}-{reference.InstanceId}-{reference.SubId}");
            var alreadyAdded = this.spawned.Any(item => item.Light?.Placed == reference);
            if (alreadyAdded)
                ImGui.BeginDisabled();
            if (ImGui.Button(alreadyAdded ? "Added" : "Add", new Vector2(64, 24)))
            {
                this.AddSpawned(new SpawnedFurniture(light));
                ImGui.CloseCurrentPopup();
            }
            if (alreadyAdded)
                ImGui.EndDisabled();
    ...
    private string DisplayNameWithDuplicateIndex(SpawnedFurniture furniture)
    {
        var displayName = furniture.IsNpc && !string.IsNullOrWhiteSpace(furniture.NpcName)
            ? furniture.NpcName
            : furniture.Name;
        var modelPath = furniture.ModelPath.Trim();
        var matchCount = 0;
        var matchIndex = 0;
        foreach (var item in this.spawned)
        {
            var sameItem = furniture.IsLight
                ? item.IsLight && item.Name.Equals(furniture.Name, StringComparison.OrdinalIgnoreCase)
                : item.ModelPath.Trim().Equals(modelPath, StringComparison.OrdinalIgnoreCase);
            if (!sameItem)
                continue;
        ...
        return matchCount > 1 ? $"{displayName} #{matchIndex}" : displayName;
    }
    ...
    private void DrawLightControls(SpawnedFurniture selected)
    {
        var light = selected.Light!;
        var settings = light.Settings;

        ImGui.Spacing();
        if (light.Placed is { } placed)
            ImGui.TextDisabled($"Placed light {placed.InstanceId}");

        ImGui.SetNextItemWidth(220);
        if (ImGui.BeginCombo("Type", SceneLighting.LightName(settings.Type)))
        {
            foreach (var type in Enum.GetValues<SceneLightType>())
            {
                if (ImGui.Selectable(SceneLighting.LightName(type), settings.Type == type))
                    settings = settings with { Type = type };
            }
            ImGui.EndCombo();
        }
        ...
        ImGui.SetNextItemWidth(260);
        if (ImGui.ColorEdit3("Color", ref color, ImGuiColorEditFlags.PickerHueWheel | ImGuiColorEditFlags.Hdr | ImGuiColorEditFlags.Float))
            settings = settings with { Color = Vector3.Clamp(color, Vector3.Zero, new Vector3(4.0f)) };

        var intensity = settings.Intensity;
        ImGui.SetNextItemWidth(220);
        if (ImGui.DragFloat("Intensity", ref intensity, 0.05f, 0.0f, 100.0f))
            settings = settings with { Intensity = Math.Clamp(intensity, 0.0f, 100.0f) };

        var range = settings.Range;
        ImGui.SetNextItemWidth(220);
        if (ImGui.DragFloat("Range", ref range, 0.1f, 0.0f, 900.0f))
            settings = settings with { Range = Math.Clamp(range, 0.0f, 900.0f) };

        ImGui.SetNextItemWidth(220);
        if (ImGui.BeginCombo("Falloff", settings.Falloff.ToString()))
        {
            foreach (var falloff in Enum.GetValues<SceneLightFalloff>())
            {
                if (ImGui.Selectable(falloff.ToString(), settings.Falloff == falloff))
                    settings = settings with { Falloff = falloff };
            }
            ImGui.EndCombo();
        }

        var falloffFactor = settings.FalloffFactor;
        ImGui.SetNextItemWidth(220);
        if (ImGui.DragFloat("Falloff power", ref falloffFactor, 0.01f, 0.0f, 1000.0f))
            settings = settings with { FalloffFactor = Math.Clamp(falloffFactor, 0.0f, 1000.0f) };

        if (settings.Type == SceneLightType.Spot)
        {
            var angle = settings.SpotAngle;
            ImGui.SetNextItemWidth(220);
            if (ImGui.SliderFloat("Cone angle", ref angle, 0.0f, 180.0f, "%.0f°"))
                settings = settings with { SpotAngle = angle };

            var edge = settings.AngularFalloff;
            ImGui.SetNextItemWidth(220);
            if (ImGui.SliderFloat("Soft edge", ref edge, 0.0f, 180.0f, "%.1f°"))
                settings = settings with { AngularFalloff = edge };
        }
        else if (settings.Type == SceneLightType.Area)
        {
            var skew = settings.AreaSkew;
            ImGui.SetNextItemWidth(260);
            if (ImGui.SliderFloat2("Skew", ref skew, -90.0f, 90.0f, "%.1f°"))
                settings = settings with { AreaSkew = skew };
        ...
        var specular = settings.SpecularHighlights;
        if (ImGui.Checkbox("Reflections", ref specular))
            settings = settings with { SpecularHighlights = specular };

        var characterShadows = settings.CharacterShadows;
        if (ImGui.Checkbox("Character shadows", ref characterShadows))
            settings = settings with { CharacterShadows = characterShadows };

        var objectShadows = settings.ObjectShadows;
        if (ImGui.Checkbox("Object shadows", ref objectShadows))
            settings = settings with { ObjectShadows = objectShadows };

        var dynamicShadows = settings.DynamicShadows;
        if (ImGui.Checkbox("Dynamic shadows", ref dynamicShadows))
            settings = settings with { DynamicShadows = dynamicShadows };
    ...
    private void DrawNpcAppearanceControls(SpawnedFurniture selected)
    {
        ...
        var appearanceStatus = this.npcSpawner.GetAppearanceStatus(selected.Id);
        ImGui.TextDisabled(appearanceStatus.Length > 0 ? appearanceStatus : " ");
    ...
    private void DrawForcedDyeControls(SpawnedFurniture selected)
    {
        ...
        var textures = this.spawner.GetTextureSlotInfos(selected.Id);
        if (textures.Count == 0)
        {
            ImGui.Unindent();
            return;
        ...
        {
            if (texture.MaterialIndex != previousMaterial)
            {
                ImGui.TextUnformatted($"Material {texture.MaterialIndex + 1}");
                previousMaterial = texture.MaterialIndex;
            }

            this.DrawForcedTextureDyeSelector(selected, texture);
        }

        ImGui.Unindent();
    }

    private void DrawForcedTextureDyeSelector(SpawnedFurniture selected, TextureSlotInfo texture)
    {
        var dye = selected.GetForcedTextureDye(texture.MaterialIndex, texture.TextureIndex);
        var preview = dye.StainId == 0 ? "Default" : this.stainCatalog.Find(dye.StainId)?.Name ?? dye.StainId.ToString();
        ImGui.SetNextItemWidth(220);
        if (ImGui.BeginCombo($"{texture.Name}##texture-{texture.MaterialIndex}-{texture.TextureIndex}", preview))
        {
            if (ImGui.Selectable("Default", dye.StainId == 0 && !dye.CustomColorEnabled))
            {
                selected.SetForcedTextureDye(texture.MaterialIndex, texture.TextureIndex, 0, false, dye.Color);
                this.spawner.InvalidateForcedDyes(selected.Id);
            }
            foreach (var stain in this.stainCatalog.Items)
            {
                if (ImGui.Selectable($"{stain.Name}##texture-{texture.MaterialIndex}-{texture.TextureIndex}-{stain.StainId}",
                        stain.StainId == dye.StainId && !dye.CustomColorEnabled))
                {
                    selected.SetForcedTextureDye(texture.MaterialIndex, texture.TextureIndex, stain.StainId, false, dye.Color);
                    this.spawner.InvalidateForcedDyes(selected.Id);
                }
            }
            ImGui.EndCombo();
        ...
        var enabled = dye.CustomColorEnabled;
        if (ImGui.Checkbox($"Custom##texture-custom-{texture.MaterialIndex}-{texture.TextureIndex}", ref enabled))
        {
            selected.SetForcedTextureDye(texture.MaterialIndex, texture.TextureIndex, enabled ? (byte)0 : dye.StainId, enabled, dye.Color);
            this.spawner.InvalidateForcedDyes(selected.Id);
        }
        ...
        if (enabled)
        {
            ...
            var color = dye.Color;
            ImGui.SetNextItemWidth(220);
            if (ImGui.ColorEdit4($"##forced-texture-color-{texture.MaterialIndex}-{texture.TextureIndex}", ref color,
                    ImGuiColorEditFlags.PickerHueWheel | ImGuiColorEditFlags.NoAlpha))
            {
                selected.SetForcedTextureDye(texture.MaterialIndex, texture.TextureIndex, 0, true, color);
                this.spawner.InvalidateForcedDyes(selected.Id);
            }
        }
    }
    ...
    {
        if (selected is null || !selected.Enabled || (!selected.IsLight && string.IsNullOrWhiteSpace(selected.ModelPath)))
            return;
        ...
        if (ImGui.Begin("Izzy's Furniture Gizmo Overlay", flags))
        {
            ...
            {
                ImGuizmo.RecomposeMatrixFromComponents(ref position.X, ref rotation.X, ref scale.X, ref matrix.M11);
                LightOverlayRenderer.Draw(selected, matrix, view, projection, io.DisplaySize);
            }
