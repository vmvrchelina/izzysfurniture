...
internal sealed class SpawnedFurniture
{
    ...
    private readonly Dictionary<ForcedTextureKey, ForcedTextureDye> forcedTextureDyes = [];
    ...
    public SpawnedFurniture(SceneLightType lightType, Vector3 position)
        : this(new SceneLight(SceneLightSettings.Create(lightType)), position, SceneLighting.LightName(lightType))
    {
    }

    internal SpawnedFurniture(SceneLight light, Vector3 position, string name)
    {
        this.Id = Guid.NewGuid();
        this.Name = name;
        this.Position = position;
    ...
    public SpawnedFurniture(PlacedLight light)
        : this(new SceneLight(light.State.Settings, light.Reference), light.State.Position, light.Name)
    {
        this.RotationDegrees = light.State.RotationDegrees;
        this.Scale3 = light.State.Scale;
        this.Enabled = light.State.Enabled;
    }
    ...
    private SpawnedFurniture(SpawnedFurniture source, Vector3 position)
    {
        ...
        // a duplicate is independent even when its source was a placed map light
        this.Light = source.Light?.CopyAsCustom();
        ...
        foreach (var (key, dye) in source.forcedTextureDyes)
            this.forcedTextureDyes[key] = dye;
    }
    ...
    public bool IsLight => this.Light is not null;
    ...
    public IReadOnlyDictionary<ForcedTextureKey, ForcedTextureDye> ForcedTextureDyes => this.forcedTextureDyes;
    ...
    public ForcedTextureDye GetForcedTextureDye(int materialIndex, int textureIndex)
    {
        var key = new ForcedTextureKey(materialIndex, textureIndex);
        if (this.forcedTextureDyes.TryGetValue(key, out var dye))
            return dye;

        // old scenes use -1 to apply a material dye to every texture in that material
        return this.forcedTextureDyes.TryGetValue(new ForcedTextureKey(materialIndex, -1), out dye)
            ? dye
            : new ForcedTextureDye(0, false, Vector4.One);
    }

    public void SetForcedTextureDye(int materialIndex, int textureIndex, byte stainId, bool customColorEnabled, Vector4 color)
    {
        this.forcedTextureDyes.Remove(new ForcedTextureKey(materialIndex, -1));
        var key = new ForcedTextureKey(materialIndex, textureIndex);
        if (stainId == 0 && !customColorEnabled)
        {
            this.forcedTextureDyes.Remove(key);
            return;
        }

        this.forcedTextureDyes[key] = new ForcedTextureDye(stainId, customColorEnabled, ColorMath.Clamp01(color));
    }

    public void SetLegacyForcedMaterialDye(int materialIndex, byte stainId, bool customColorEnabled, Vector4 color)
    {
        if (stainId == 0 && !customColorEnabled)
            return;

        this.forcedTextureDyes[new ForcedTextureKey(materialIndex, -1)] =
            new ForcedTextureDye(stainId, customColorEnabled, ColorMath.Clamp01(color));
    }

    // zero represents the unmodified dye state
    ...
    public ulong ForcedDyeStateSignature(int materialCount)
    {
        ...
        foreach (var (key, dye) in this.forcedTextureDyes.OrderBy(item => item.Key.MaterialIndex).ThenBy(item => item.Key.TextureIndex))
        {
            if (key.MaterialIndex < 0 || key.MaterialIndex >= count)
                continue;

            signature = (signature ^ (uint)key.MaterialIndex) * 1099511628211UL;
            signature = (signature ^ unchecked((uint)key.TextureIndex)) * 1099511628211UL;
            var value = dye.CustomColorEnabled
                ? 0x100000000UL | PackColor(dye.Color)
                : dye.StainId;
...
internal readonly record struct ForcedTextureKey(int MaterialIndex, int TextureIndex);
internal readonly record struct ForcedTextureDye(byte StainId, bool CustomColorEnabled, Vector4 Color);
