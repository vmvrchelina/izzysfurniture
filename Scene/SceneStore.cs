...
internal sealed class SceneStore
{
    ...
    private SpawnedFurniture? ToSpawned(SavedProp saved)
    {
        ...
        if (saved.ForcedTextureDyes.Any(dye =>
                dye.MaterialIndex < 0 ||
                dye.MaterialIndex >= SpawnedFurniture.MaxForcedDyeMaterials ||
                dye.TextureIndex < -1 ||
                !dye.Color.IsFinite) ||
            saved.ForcedTextureDyes.Select(dye => (dye.MaterialIndex, dye.TextureIndex)).Distinct().Count() != saved.ForcedTextureDyes.Count)
        {
            return null;
        }

        if (string.Equals(saved.Kind, SavedProp.LightKind, StringComparison.OrdinalIgnoreCase))
        {
            if (saved.Light is null || !saved.Light.TryCreate(out var light))
                return null;

            var prop = new SpawnedFurniture(light, saved.Position.ToVector3(), saved.Name);
    ...
    private static void ApplySavedTransform(SpawnedFurniture prop, SavedProp saved)
    {
        ...
        foreach (var dye in saved.ForcedTextureDyes)
            prop.SetForcedTextureDye(dye.MaterialIndex, dye.TextureIndex, dye.StainId, dye.CustomColorEnabled, dye.Color.ToVector4());

        // material-wide entries are the representation used by scenes saved before texture dyes
        var legacyCount = Math.Max(saved.ForcedMaterialStainIds.Count,
            Math.Max(saved.ForcedMaterialDyeColors.Count, saved.ForcedMaterialDyeColorEnabled.Count));
        for (var materialIndex = 0; materialIndex < legacyCount; materialIndex++)
        {
            var stainId = materialIndex < saved.ForcedMaterialStainIds.Count ? saved.ForcedMaterialStainIds[materialIndex] : (byte)0;
            var custom = materialIndex < saved.ForcedMaterialDyeColorEnabled.Count && saved.ForcedMaterialDyeColorEnabled[materialIndex];
            var color = materialIndex < saved.ForcedMaterialDyeColors.Count
                ? saved.ForcedMaterialDyeColors[materialIndex].ToVector4()
                : Vector4.One;
            prop.SetLegacyForcedMaterialDye(materialIndex, stainId, custom, color);
        }
    ...
    private sealed class SavedProp
    {
        ...
        public List<SavedForcedTextureDye> ForcedTextureDyes { get; set; } = [];
        public SavedColor FxColor { get; set; } = SavedColor.FromVector4(Vector4.One);

        public SavedLight? Light { get; set; }
        ...
        public static SavedProp FromSpawned(SpawnedFurniture prop)
        {
            var saved = new SavedProp
            {
                ...
                ForcedTextureDyes = prop.ForcedTextureDyes
                    .Select(item => SavedForcedTextureDye.From(item.Key, item.Value))
                    .ToList(),
            ...
            if (prop.Light is { } light)
            {
                saved.Kind = LightKind;
                saved.Light = SavedLight.From(light);
            }
    ...
    private sealed class SavedForcedTextureDye
    {
        public int MaterialIndex { get; set; }
        public int TextureIndex { get; set; }
        public byte StainId { get; set; }
        public bool CustomColorEnabled { get; set; }
        public SavedColor Color { get; set; } = SavedColor.FromVector4(Vector4.One);

        public static SavedForcedTextureDye From(ForcedTextureKey key, ForcedTextureDye dye)
            => new()
            {
                MaterialIndex = key.MaterialIndex,
                TextureIndex = key.TextureIndex,
                StainId = dye.StainId,
                CustomColorEnabled = dye.CustomColorEnabled,
                Color = SavedColor.FromVector4(dye.Color),
            };
    }

    private sealed class SavedLight
    {
        public SceneLightSource? Source { get; set; }
        public uint TerritoryId { get; set; }
        public uint InstanceId { get; set; }
        public uint SubId { get; set; }
        public SceneLightType Type { get; set; }
        public SavedVector3 Color { get; set; } = SavedVector3.FromVector3(Vector3.One);
        public float Intensity { get; set; } = 1.0f;
        public float Range { get; set; } = 8.0f;
        public SceneLightFalloff Falloff { get; set; } = SceneLightFalloff.Quadratic;
        public float FalloffFactor { get; set; } = 1.0f;
        public float SpotAngle { get; set; } = 45.0f;
        public float AngularFalloff { get; set; } = 0.5f;
        public SavedVector2 AreaSkew { get; set; }
        public bool SpecularHighlights { get; set; } = true;
        public bool DynamicShadows { get; set; }
        public bool CharacterShadows { get; set; }
        public bool ObjectShadows { get; set; }

        public static SavedLight From(SceneLight light)
        {
            var settings = light.Settings;
            var saved = new SavedLight
            {
                Type = settings.Type,
                Color = SavedVector3.FromVector3(settings.Color),
                Intensity = settings.Intensity,
                Range = settings.Range,
                Falloff = settings.Falloff,
                FalloffFactor = settings.FalloffFactor,
                SpotAngle = settings.SpotAngle,
                AngularFalloff = settings.AngularFalloff,
                AreaSkew = SavedVector2.FromVector2(settings.AreaSkew),
                SpecularHighlights = settings.SpecularHighlights,
                DynamicShadows = settings.DynamicShadows,
                CharacterShadows = settings.CharacterShadows,
                ObjectShadows = settings.ObjectShadows,
            };

            if (light.Placed is { } placed)
            {
                saved.Source = placed.Source;
                saved.TerritoryId = placed.TerritoryId;
                saved.InstanceId = placed.InstanceId;
                saved.SubId = placed.SubId;
            }

            return saved;
        }

        public bool TryCreate(out SceneLight light)
        {
            var valid = Enum.IsDefined(this.Type) &&
                Enum.IsDefined(this.Falloff) &&
                this.Color.IsNonNegativeFinite &&
                this.AreaSkew.IsBetween(-90.0f, 90.0f) &&
                IsNonNegative(this.Intensity) &&
                IsNonNegative(this.Range) &&
                IsNonNegative(this.FalloffFactor) &&
                IsAngle(this.SpotAngle) &&
                IsAngle(this.AngularFalloff) &&
                (this.Source is null || Enum.IsDefined(this.Source.Value));
            ...
            PlacedLightReference? placed = this.Source is { } source
                ? new PlacedLightReference(source, this.TerritoryId, this.InstanceId, this.SubId)
                : null;
            var settings = new SceneLightSettings(
                this.Type,
                this.Color.ToVector3(),
                this.Intensity,
                this.Range,
                this.Falloff,
                this.FalloffFactor,
                this.SpotAngle,
                this.AngularFalloff,
                this.AreaSkew.ToVector2(),
                this.SpecularHighlights,
                this.DynamicShadows,
                this.CharacterShadows,
                this.ObjectShadows);

            light = new SceneLight(settings, placed);
            return true;
        }
        ...
        private static bool IsAngle(float value)
            => float.IsFinite(value) && value >= 0.0f && value <= 180.0f;
    }
    ...
    private readonly record struct SavedVector3(float X, float Y, float Z)
    {
        ...
        public bool IsNonNegativeFinite => this.IsFinite && this.X >= 0.0f && this.Y >= 0.0f && this.Z >= 0.0f;
    ...
    {
        public bool IsFinite => float.IsFinite(this.R) && float.IsFinite(this.G) && float.IsFinite(this.B) && float.IsFinite(this.A);
    ...
    private readonly record struct SavedVector2(float X, float Y)
    {
        public bool IsFinite => float.IsFinite(this.X) && float.IsFinite(this.Y);
        public bool IsBetween(float minimum, float maximum)
            => this.IsFinite &&
                this.X >= minimum && this.X <= maximum &&
                this.Y >= minimum && this.Y <= maximum;

        public static SavedVector2 FromVector2(Vector2 value)
            => new(value.X, value.Y);
